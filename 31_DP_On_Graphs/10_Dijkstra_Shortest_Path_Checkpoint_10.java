...
class DijkstraShortestPathCheckpoint10 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int n = fs.nextInt(), m = fs.nextInt(), src = fs.nextInt(), dst = fs.nextInt();
        List<List<E>> g = new ArrayList<>();
        for (int i = 0; i <= n; i++) g.add(new ArrayList<>());
        for (int i = 0; i < m; i++) { int u = fs.nextInt(), v = fs.nextInt(), w = fs.nextInt(); g.get(u).add(new E(v, w)); g.get(v).add(new E(u, w)); }
        long[] d = new long[n + 1];
